...
for(const folder of (await readdir(base,{withFileTypes:true})).filter(e=>e.isDirectory()&&e.name.startsWith('walking_')).map(e=>e.name).sort()){
 const files=(await readdir(new URL(folder+'/',base))).filter(f=>new RegExp(`^${folder}_\\d+\\.webp$`).test(f)).sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));
 if(!files.length)throw new Error(`No frames in ${folder}`);
 files.forEach((f,i)=>{if(f!==`${folder}_${i+1}.webp`)throw new Error(`Missing or out-of-order frame in ${folder}: expected ${i+1}`)});
